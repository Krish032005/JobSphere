        ...
        <div className='bg-(--color-surface) border border-(--color-border) rounded-lg p-8 flex justify-between items-start'>
          <div className='flex items-center gap-6'>
            ...
            <div>
              ...
              <p className='font-semibold text-(--color-text-hover) mt-1'>Frontend Engineer · Mumbai, IN</p>
              <div className='flex items-center gap-3 mt-3'>
                <span className='text-sm font-semibold px-3 py-1 rounded-full bg-(--color-surface-secondary) text-(--color-text)'>
                  Open to work
                </span>
                <span className='text-sm font-semibold text-(--color-border)'>
                  4 years experience</span>
              </div>
            </div>
          </div>

          <button className='font-semibold text-sm bg-(--color-btn) text-(--color-text) rounded-lg px-5 py-2 cursor-pointer active:scale-95'>
            Edit profile
          </button>
        </div>
        ...
        <div className='grid grid-cols-3 gap-8'>
          ...
          <div className='col-span-2 flex flex-col gap-8'>
            ...
            <div className='bg-(--color-surface) border border-(--color-border) rounded-lg p-8'>
              <h2 className='font-bold text-xl text-(--color-text-hover) mb-4'>About</h2>
              <p className='font-semibold text-(--color-text) leading-relaxed'>
            ...
            {/* Experience */}
            <div className='bg-(--color-surface) border border-(--color-border) rounded-lg p-8'>
              <h2 className='font-bold text-xl text-(--color-text-hover) mb-6'>Experience</h2>
                ...
                {experience.map((exp, i) => (
                  <div key={i} className='flex gap-4 pb-6 border-b border-(--color-border) last:border-0 last:pb-0'>
                    ...
                    <div>
                      ...
                      <p className='font-semibold text-(--color-border)'>{exp.company} · {exp.duration}</p>
                      <p className='font-semibold text-(--color-border) mt-2'>{exp.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Education */}
            <div className='bg-(--color-surface) border border-(--color-border) rounded-lg p-8'>
              <h2 className='font-bold text-xl text-(--color-text-hover) mb-6'>Education</h2>
                ...
                {education.map((edu, i) => (
                  <div key={i} className='flex gap-4'>
                    ...
                    <div>
                      ...
                      <p className='font-semibold text-(--color-border)'>{edu.school} · {edu.duration}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className='flex flex-col gap-8'>
            ...
            <div className='bg-(--color-surface) border border-(--color-border) rounded-lg p-6'>
              <h2 className='font-bold text-lg text-(--color-text-hover) mb-4'>Skills</h2>
              <div className='flex flex-wrap gap-2'>
                {skills.map((skill) => (
                  <span key={skill} className='text-sm font-semibold px-3 py-1 rounded-full bg-(--color-navbar) text-(--color-text)'>
                    {skill}
                  </span>
                ))}
              </div>
            ...
            {/* Resume */}
            <div className='bg-(--color-surface) border border-(--color-border) rounded-lg p-6'>
              <h2 className='font-bold text-lg text-(--color-text-hover) mb-4'>Resume</h2>
              <div className='border border-dashed border-(--color-border) rounded-lg p-6 flex flex-col items-center gap-3 text-center'>
                ...
                <p className='text-sm font-semibold text-(--color-border)' >Uploaded 3 weeks ago</p>
                <button className='font-semibold text-sm text-(--color-text) cursor-pointer bg-(--color-navbar) rounded-full px-3 py-1'>Replace file</button>
              </div>
            </div>

            {/* Contact */}
            <div className='bg-(--color-surface) border border-(--color-border) rounded-lg p-6'>
              <h2 className='font-bold text-lg text-(--color-text-hover) mb-4'>Contact</h2>
              <div className='flex flex-col gap-3'>
                <p className='font-semibold text-(--color-text)'>ananya.rao@email.com</p>
                <p className='font-semibold text-(--color-text)'>+91 98765 43210</p>
              </div>
            </div>
          </div>
        </div>
